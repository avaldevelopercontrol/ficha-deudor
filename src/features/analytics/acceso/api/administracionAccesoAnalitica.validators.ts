...
} from './administracionAccesoAnalitica.dto';
import {
...
const parsePublication = (
  ...
  const candidateGroups = expectAnalyticsArray(
    record.gruposCandidatos ?? record.candidateGroups,
    contract,
  ...
  const groupIds = parsePositiveIntegerArray(
    record.idsGrupos ?? record.groupIds,
    contract,
    `${path}.groupIds`
  );
  ...
  return {
    ...
    isAvailable: expectAnalyticsBoolean(
      record.estaDisponible ?? record.isAvailable,
      contract,
      `${path}.isAvailable`
    ),
    ...
    hasExplicitGroupConfiguration: expectAnalyticsBoolean(
      record.tieneConfiguracionGruposExplicita ?? record.hasExplicitGroupConfiguration,
      contract,
    ...
    isReady: expectAnalyticsBoolean(
      record.estaLista ?? record.isReady,
      contract,
      `${path}.isReady`
    ),
  };
...
export const parseAnalyticsPowerBiConfigurationDto = (
  ...
  return {
    ...
    isConfigured: expectAnalyticsBoolean(
      record.estaConfigurada ?? record.isConfigured,
      contract,
