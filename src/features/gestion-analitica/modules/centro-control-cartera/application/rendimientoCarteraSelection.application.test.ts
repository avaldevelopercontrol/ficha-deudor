...
} from './rendimientoCarteraSelection.application';
...
export const suite = defineSuite(
  'rendimientoCarteraSelection.application',
  [
    test(
      'mantiene como efectivo un supervisor disponible en el contexto',
      () => {
        const result = resolveRendimientoCarteraSelection({
    ...
    test(
      ...
      () => {
        ...
        assert.deepEqual(
          ...
          ['10', CARTERA_UNASSIGNED_SUPERVISOR_FILTER_ID]
        );
      }
    ),
    test(
