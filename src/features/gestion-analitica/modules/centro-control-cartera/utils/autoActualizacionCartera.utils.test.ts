...
import {
  CARTERA_AUTO_REFRESH_JITTER_MS,
  CARTERA_AUTO_REFRESH_MS,
  scheduleAutoActualizacionCartera,
  type AutoActualizacionCarteraTimers,
} from './autoActualizacionCartera.utils';
...
export const suite = defineSuite(
  'autoActualizacionCartera.utils',
  [
    test(
      ...
      () => {
        ...
        assert.deepEqual(fixture.timeoutDelays, [
          Math.floor(CARTERA_AUTO_REFRESH_JITTER_MS * 0.5),
        ]);
    ...
    test(
      ...
      () => {
        const fixture = createTimerFixture();
        const cleanup = scheduleAutoActualizacionCartera({
          refetch: () => undefined,
          isVisible: () => true,
          timers: fixture.timers,
          random: () => 0,
