...
const OVERDUE_SORT_KEYS = new Set<PromesasCarteraVencidasSortKey>([
  'debtorId',
  'dueDate',
  'overdueDays',
  'promiseAmount',
...
const DUE_TODAY_SORT_KEYS = new Set<PromesasCarteraVenceHoySortKey>([
  'debtorId',
  'promiseAmount',
  'paidAmount',
  'outstandingAmount',
...
const PROMISE_TRACKING_STATUS_KEYS = new Set<SeguimientoPromesasCarteraStatusKey>([
  'pending',
  'partial',
  'fulfilled',
  'broken',
...
const PROMISE_TRACKING_SORT_KEYS = new Set<SeguimientoPromesasCarteraSortKey>([
  'debtorId',
  'promiseAmount',
  'paidAmount',
  'outstandingAmount',
...
export const validateSeguimientoPromesasQuery = (
  query: SeguimientoPromesasCarteraQuery
): void => {
  normalizeIsoDateParam('dueDate', query.dueDate);
  assertPositiveIntegerParam('page', query.page);
  assertPositiveIntegerParam('pageSize', query.pageSize);

  if (
    query.status !== null &&
    !PROMISE_TRACKING_STATUS_KEYS.has(query.status)
  ) {
    throw new Error('status no es un valor soportado para seguimiento de promesas.');
  }

  if (!PROMISE_TRACKING_SORT_KEYS.has(query.sortBy)) {
    throw new Error('sortBy no es un criterio soportado para seguimiento de promesas.');
  }
