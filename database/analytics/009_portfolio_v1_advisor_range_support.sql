...
Soporte para métricas exactas de asesor en rangos multi-día
Motor: SQL Server
...
/* ============================================================
   Contacto por asesor/deudor/día

   Grain:
   date + client + campaign + portfolio + advisor + debtor

   Motivo:
   fact_advisor_daily contiene distinct diarios. Sumar esos distinct
   en un rango puede duplicar al mismo deudor entre fechas.

   Esta fact conserva el grain necesario para reagrupar un rango y
   aplicar precedencia CD > CI > NC antes de contar deudores.
   ============================================================ */

IF OBJECT_ID('analytics.fact_advisor_debtor_contact_daily', 'U') IS NULL
BEGIN
    CREATE TABLE analytics.fact_advisor_debtor_contact_daily
    (
        advisor_debtor_contact_daily_key BIGINT IDENTITY(1,1) NOT NULL
            CONSTRAINT PK_fact_advisor_debtor_contact_daily PRIMARY KEY,
        ...
        loaded_at                DATETIME2(3) NOT NULL
            CONSTRAINT DF_fact_advisor_debtor_contact_daily_loaded_at
            DEFAULT (SYSUTCDATETIME()),

        CONSTRAINT FK_fact_advisor_debtor_contact_daily_date
            FOREIGN KEY (date_key)
            REFERENCES analytics.dim_date(date_key),

        CONSTRAINT FK_fact_advisor_debtor_contact_daily_client
            FOREIGN KEY (client_key)
            REFERENCES analytics.dim_client(client_key),

        CONSTRAINT FK_fact_advisor_debtor_contact_daily_campaign
            FOREIGN KEY (campaign_key)
            REFERENCES analytics.dim_campaign(campaign_key),

        CONSTRAINT FK_fact_advisor_debtor_contact_daily_portfolio
            FOREIGN KEY (portfolio_key)
            REFERENCES analytics.dim_portfolio(portfolio_key),

        CONSTRAINT FK_fact_advisor_debtor_contact_daily_advisor
...
(
    SELECT 1
    FROM sys.indexes
    WHERE object_id = OBJECT_ID('analytics.fact_advisor_debtor_contact_daily')
      AND name = 'UX_fact_advisor_debtor_contact_daily_grain'
)
BEGIN
    CREATE UNIQUE INDEX UX_fact_advisor_debtor_contact_daily_grain
        ON analytics.fact_advisor_debtor_contact_daily
        (
            date_key,
            client_key,
            campaign_key,
...
(
    ...
    FROM sys.indexes
    WHERE object_id = OBJECT_ID('analytics.fact_advisor_debtor_contact_daily')
      AND name = 'IX_fact_advisor_debtor_contact_daily_range'
)
BEGIN
    CREATE INDEX IX_fact_advisor_debtor_contact_daily_range
        ON analytics.fact_advisor_debtor_contact_daily
        (
            client_key,
            campaign_key,
...
CREATE OR ALTER VIEW analytics.v_advisor_debtor_contact_daily
AS
SELECT
